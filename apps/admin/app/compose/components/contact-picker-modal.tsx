...
export default function ContactPickerModal({
  ...
  const [selected, setSelected] = useState<Set<number>>(() => {
    ...
    for (const r of currentRecipients) {
      const c = contacts.find(
        (ct) => ct.email.toLowerCase() === r.email.toLowerCase(),
      );
      if (c) ids.add(c.id);
    }
    return ids;
  ...
  const currentFieldEmails = new Set(
    currentRecipients.map((r) => r.email.toLowerCase()),
  );
  ...
  const handleConfirm = () => {
    ...
    const rawRecipients = currentRecipients.filter(
      (r) =>
        !r.isContact &&
  ...
  const allFilteredSelected =
    filtered
      .filter((c) => !isUsedElsewhere(c))
      .every((c) => selected.has(c.id)) && filtered.length > 0;
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Adding to{" "}
                <span className="font-medium uppercase">{targetField}</span>{" "}
          ...
          {loading ? (
            ...
            <div className="py-1">
              {grouped.map(([group, groupContacts]) => {
                const availableInGroup = groupContacts.filter(
                  (c) => !isUsedElsewhere(c),
                );
                const selectedInGroup = availableInGroup.filter((c) =>
                  selected.has(c.id),
                ).length;
                const isCollapsed = collapsedGroups.has(group);
                const allGroupSelected =
                  availableInGroup.length > 0 &&
                  availableInGroup.every((c) => selected.has(c.id));
                        ...
                        <span className="text-[11px] text-muted-foreground">
                          {selectedInGroup > 0 && (
                            <span className="text-blue-600">
                              {selectedInGroup} selected &middot;{" "}
                            </span>
                          )}
                      ...
                      groupContacts.map((c) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span
                              className={`inline-flex items-center justify-center h-9 w-9 rounded-full text-white text-sm font-bold shrink-0 ${avatarColor(c.name)}`}
                            >
