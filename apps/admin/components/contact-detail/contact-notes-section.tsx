...
export default function ContactNotesSection({
  ...
  const handleUpdateTodoDue = async (
    noteId: number,
    dateStr: string | null,
  ) => {
    try {
      const res = await submissionsApi.updateTodoDue(
        contactId,
        noteId,
        dateStr,
      );
      ...
      setNotes((prev) =>
        prev.map((n) =>
          n.id === noteId ? { ...n, due_at: updated.due_at } : n,
        ),
      );
    } catch {
      /* silently fail */
          ...
          {notes.map((note) => {
            if (note.type === "system") {
              return (
                <div
                  key={note.id}
                  className="text-sm rounded px-3 py-2 bg-muted text-muted-foreground italic"
                >
                  <p>{note.content}</p>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-2 flex-1 min-w-0">
                    ...
                    <p
                      className={`whitespace-pre-wrap flex-1 ${isDone ? "line-through text-muted-foreground" : ""}`}
                    >
                      {note.content}
                    </p>
                  </div>
                ...
                <div className="text-xs text-muted-foreground mt-1 ml-6 flex items-center flex-wrap gap-x-1.5">
                  ...
                  {subtype !== "note" && (
                    ...
                    >
                      {subtype === "todo"
                        ? isDone
                          ? `Done${note.completed_at ? ` ${formatDate(note.completed_at)}` : ""}`
                          : "Todo"
                        : subtype === "reply"
                          ? "Reply from contact"
                          : "Message sent"}
                    ...
                    (() => {
                      ...
                      if (dueAt) {
                        const diff = Math.ceil(
                          (new Date(dueAt).getTime() - Date.now()) / 86400000,
                        );
                        const isOverdue = diff < 0 && !isDone;
                        const isToday = diff === 0;
                        const dateLabel = new Date(dueAt).toLocaleDateString(
                          "en-US",
                          ...
                          <span className="inline-flex items-center gap-0.5">
                            <button
                              ...
                              onClick={(e) =>
                                openDatePicker(
                                  dueAt.split("T")[0],
                                  (v) => void handleUpdateTodoDue(note.id, v),
                                  e,
                                )
                              }
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                void handleUpdateTodoDue(note.id, null)
                              }
                ...
                {todoDueDate ? (
                  <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium bg-accent text-muted-foreground border border-border">
                    <button
                      type="button"
                      onClick={(e) =>
                        openDatePicker(todoDueDate, (v) => setTodoDueDate(v), e)
                      }
                    ...
                    >
                      ...
                      Due{" "}
                      {new Date(todoDueDate + "T00:00").toLocaleDateString(
                        "en-US",
                        { month: "short", day: "numeric" },
                      )}
                    </button>
                  ...
                  <button
                    type="button"
                    onClick={(e) =>
                      openDatePicker("", (v) => setTodoDueDate(v), e)
                    }
