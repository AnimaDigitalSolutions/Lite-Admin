...
class AhasendProvider {
  ...
  async sendDirect(
    ...
    try {
      ...
      logger.info({
        message: `Direct email sent via AhaSend to ${to.email}`,
        data: { subject },
      });
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      logger.error({
        message: `Failed to send direct email via AhaSend: ${msg}`,
      });
      throw new Error(`Email sending failed: ${msg}`);
    }
  }
  ...
  async sendCampaign(
    ...
    try {
      ...
      logger.info({
        message: `Campaign email sent via AhaSend to ${subscriber.email}`,
      });
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      logger.error({
        message: `Failed to send campaign email via AhaSend: ${msg}`,
      });
