...
interface ContactsCalendarProps {
  ...
  setCalendarMonth: React.Dispatch<
...
export default function ContactsCalendar({
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(sortedContacts.length / ITEMS_PER_PAGE),
  );
      ...
      result.push({
        ...
        monthLabel: showMonth
          ? d.toLocaleDateString("en-US", {
              month: "short",
              year: d.getMonth() === 0 ? "numeric" : undefined,
            })
          : undefined,
      });
  ...
  useEffect(() => {
    if (todayLineRef.current && scrollRef.current) {
      ...
      scrollRef.current.scrollLeft = Math.max(
        0,
        todayOffsetPx - scrollRef.current.clientWidth / 2,
      );
    }
  ...
  const headerLabel = useMemo(() => {
    const startMonth = windowStart.toLocaleDateString("en-US", {
      month: "short",
      year: "numeric",
    });
    const endMonth = windowEnd.toLocaleDateString("en-US", {
      month: "short",
      year: "numeric",
    });
    if (windowDays <= 14) {
      // Weekly view: show exact date range
      const startStr = windowStart.toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
      });
      const endStr = windowEnd.toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
      });
            ...
            <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
              {Object.entries(STATUS_BAR_COLORS).map(([status, color]) => (
                <span key={status} className="flex items-center gap-1">
                  <span
                    className={`h-2 w-6 rounded ${color} inline-block ${TERMINAL_STATUSES.has(status) ? "opacity-70" : ""}`}
                  />
                  {status
                    .replace("_", " ")
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-2 px-3 cursor-pointer transition-colors ${
                        isSelected
                          ? "bg-amber-100 relative z-50 border-l-[3px] border-l-amber-400"
                          : idx % 2 === 0
                            ? "bg-white"
                            : "bg-muted/50"
                      } hover:bg-accent`}
                    ...
                    >
                      <span
                        className={`h-2 w-2 rounded-full shrink-0 ${STATUS_BAR_COLORS[contact.status || "new"]}`}
                      />
              ...
              <div ref={scrollRef} className="overflow-x-auto flex-1">
                ...
                >
                  ...
                  <div className="h-[52px] border-b border-border relative">
                    ...
                    {(() => {
                      ...
                      while (cursor <= windowEnd) {
                        ...
                        const clampStart =
                          mStart < windowStart ? windowStart : mStart;
                        const clampEnd = mEnd > windowEnd ? windowEnd : mEnd;
                        const startPx =
                          daysBetween(windowStart, clampStart) * DAY_WIDTH;
                        const widthPx =
                          (daysBetween(clampStart, clampEnd) + 1) * DAY_WIDTH;
                        months.push({
                      ...
                      return months.map((m, i) => (
                        ...
                        >
                          <span className="text-xs font-medium text-muted-foreground px-2 truncate">
                            {m.label}
                          </span>
                        </div>
                      ));
                  ...
                  {paginatedContacts.map((contact, idx) => {
                    ...
                    const barStartDate =
                      submitDate < windowStart ? windowStart : submitDate;
                    const barEndDate = today > windowEnd ? windowEnd : today;

                    const startPx =
                      daysBetween(windowStart, barStartDate) * DAY_WIDTH;
                    const totalBarDays = Math.max(
                      1,
                      daysBetween(barStartDate, barEndDate) + 1,
                    );
                    ...
                    if (history && history.length > 0) {
                      ...
                      for (const change of history) {
                        ...
                        const clampedFrom =
                          prevDate < windowStart ? windowStart : prevDate;
                        const clampedTo =
                          changeDate > barEndDate ? barEndDate : changeDate;
                      ...
                      segments = transitions.map((t) => {
                        const segStartPx =
                          daysBetween(windowStart, t.from) * DAY_WIDTH;
                        const segDays = Math.max(
                          1,
                          daysBetween(t.from, t.to) + 1,
                        );
                        return {
                          status: t.status,
                          leftPx: segStartPx,
