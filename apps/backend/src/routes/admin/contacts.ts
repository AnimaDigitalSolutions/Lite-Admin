...
import {
  ...
  contactNoteDueSchema,
  contactSendEmailSchema,
  contactManualCreateSchema,
  bulkDeleteSchema,
  statusHistorySchema,
  activityQuerySchema,
  idParamSchema,
  ...
  async (req, res, next) => {
    try {
      ...
      if (!updated)
        return res
          .status(404)
          .json({ error: { message: "Submission not found", status: 404 } });
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const updated = await db.contacts.updateFollowUp(
        id,
        follow_up_at ?? null,
      );
      ...
      await db.adminLogs.create({
        ...
        resource: "contacts",
        resource_id: id,
        details: follow_up_at
          ? `Follow-up set to ${follow_up_at}`
          : "Follow-up cleared",
        ip_address: req.ip,
      });
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      return res.json({
        success: true,
        data: { notes, submissions, followUps },
      });
    } catch (error) {
      next(error);
    }
  ...
  async (req, res, next) => {
    try {
      ...
      if (!note)
        return res
          .status(404)
          .json({ error: { message: "Note not found", status: 404 } });
      if (note.type === "system")
        return res.status(403).json({
          error: { message: "System notes cannot be deleted", status: 403 },
        });
      if (note.contact_id !== id)
        return res.status(404).json({
          error: { message: "Note not found for this contact", status: 404 },
        });
...
router.patch(
  "/submissions/:id/notes/:noteId/toggle",
  validateParams(noteIdParamsSchema),
  async (req, res, next) => {
    try {
      const { id, noteId } = req.validatedParams;
      ...
      if ((note as { subtype?: string }).subtype !== "todo")
        return res.status(400).json({
          error: { message: "Only todos can be toggled", status: 400 },
        });
...
router.patch(
  "/submissions/:id/notes/:noteId/due",
  validateParams(noteIdParamsSchema),
  ...
  async (req, res, next) => {
    try {
      ...
      if ((note as { subtype?: string }).subtype !== "todo")
        return res.status(400).json({
          error: { message: "Only todos can have due dates", status: 400 },
        });
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const db = await DatabaseService.getInstance();
      const notes =
        await db.contactNotes.findStatusChangesByContactIds(limitedIds);
  ...
  async (req, res, _next) => {
    try {
      ...
      if (!contact)
        return res
          .status(404)
          .json({ error: { message: "Contact not found", status: 404 } });
      ...
      return res.json({
        success: true,
        data: { email_sent: true, to: contactEmail },
      });
    } catch (error) {
      const msg =
  ...
  async (req, res, next) => {
    try {
      ...
      await db.adminLogs.create({
        ...
        resource: "contacts",
        details: `Bulk deleted ${deleted} contact(s)`,
        ip_address: req.ip,
      });
