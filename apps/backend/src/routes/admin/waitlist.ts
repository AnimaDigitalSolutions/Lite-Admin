...
import {
  bulkDeleteSchema,
  idParamSchema,
  waitlistManualCreateSchema,
  waitlistUpdateSchema,
} from "../../schemas/admin.js";
import {
  waitlistListQuerySchema,
  waitlistTestEmailSchema,
  ...
  async (req, res, next) => {
    try {
      ...
      if (!updated)
        return res
          .status(404)
          .json({ error: { message: "Subscriber not found", status: 404 } });
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      await db.adminLogs.create({
        action: "waitlist_bulk_delete",
        resource: "waitlist",
        details: `Bulk deleted ${deleted} waitlist entry(s)`,
        ip_address: req.ip,
      });
