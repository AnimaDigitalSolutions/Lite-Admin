...
function shouldSkipRateLimit(req: Request): boolean {
  ...
  if (
    ...
    process.env.RATE_LIMIT_BYPASS_DEV === "true"
  ) {
    return true;
  }
...
function getFormsLimiter(max: number, windowMs: number) {
  ...
  _formsLimiter = createLimiter(
    { max, windowMs, message: rateLimitConfigs.forms.message },
    "forms",
  );
...
async function resolveFormsLimiter() {
  try {
    ...
    const max =
      config.env === "development" ? 1000 : settings.getRateLimitFormsMax();
 ...
 */
async function selectRateLimiter(
  req: Request,
  type: "general" | "forms" = "general",
) {
 ...
 */
export function suspiciousActivityBlocker(
  req: Request,
  res: Response,
  next: NextFunction,
) {
