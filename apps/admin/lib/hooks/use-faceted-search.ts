...
export function useFacetedSearch({ suggestions }: UseFacetedSearchOptions) {
  ...
  const {
    ...
    text: searchText,
  } = useMemo(() => parseSearch(searchTerm), [searchTerm]);
  ...
  const toggleFilter = (token: string, e?: React.MouseEvent) => {
    ...
    if (tokens.has(token)) {
      const replaced = searchTerm.replace(
      ...
      setSearchTerm(
        searchTerm
          .replace(new RegExp(`\\s*${negEscaped}\\s*`, "gi"), " ")
          .trim(),
      );
    } else if (additive) {
  ...
  const onInputKeyDown = (e: React.KeyboardEvent) => {
    ...
    } else if (e.key === "Enter" && filteredSuggestions[suggestionIndex]) {
      e.preventDefault();
      applySuggestion(filteredSuggestions[suggestionIndex].token);
