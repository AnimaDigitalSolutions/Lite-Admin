...
const DEFAULT_TEST_DATA = {
  ...
  message:
    "This is a test message from the admin dashboard to verify email functionality is working correctly.",
};
  ...
  return (
    <Card>
      ...
      >
        ...
        <ChevronDownIcon
          className={`h-5 w-5 shrink-0 text-muted-foreground transition-transform duration-200 ${show ? "rotate-180" : ""}`}
        />
      </button>
      {show && (
        <CardContent className="space-y-4 border-t border-border pt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setData((prev) => ({ ...prev, test_email: e.target.value }))
                }
          ...
          {result && (
            <div
              className={`p-3 rounded-md ${result.success ? "bg-green-50 text-green-800 border border-green-200" : "bg-red-50 text-red-800 border border-red-200"}`}
            >
              {result.message}
            </div>
          )}
