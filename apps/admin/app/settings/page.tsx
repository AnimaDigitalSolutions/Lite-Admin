...
const menuStructure: {
  group: string;
  items: { name: string; key: string | null; icon: IconComponent }[];
}[] = [
  {
    group: "Overview",
    items: [{ name: "Dashboard", key: null, icon: HomeIcon }],
  },
  {
    group: "Content",
    items: [{ name: "Media", key: "nav_visible_media", icon: PhotoIcon }],
  },
  {
    group: "Leads",
    items: [
  ...
  },
  {
    ...
    items: [
      { name: "Email", key: "nav_visible_email", icon: AtSymbolIcon },
      {
        ...
        key: "nav_visible_email_templates",
...
export default function SettingsPage() {
  ...
  const resetMenu = async () => {
    const updates: Record<string, boolean> = {};
    for (const g of menuStructure)
      for (const i of g.items) if (i.key) updates[i.key] = true;
    setMenuPrefs((prev) => ({ ...prev, ...updates }));
    try {
      await menuApi.update(updates);
      notifySidebar();
  ...
  const handleSaveStorage = async () => {
    ...
    try {
      ...
      setActionError("Failed to save storage config.");
    } finally {
        ...
        <div className="max-w-2xl space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Master switch for outgoing emails. Configure provider and
                addresses in the Email section.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    When off, forms save to DB but no emails are sent.
                  </p>
                </div>
                <Toggle
                  checked={settings.email_enabled}
                  onChange={(v) =>
                    setSettings((p) => ({ ...p, email_enabled: v }))
                  }
                  accent="bg-blue-600"
                />
              </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Block all public form submissions with a custom message.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Returns 503 on all contact + waitlist POSTs.
                  </p>
                </div>
                <Toggle
                  checked={settings.maintenance_mode}
                  onChange={(v) =>
                    setSettings((p) => ({ ...p, maintenance_mode: v }))
                  }
              ...
              {settings.maintenance_mode && (
                <div className="rounded-md border border-orange-200 bg-orange-50 px-4 py-3 text-sm text-orange-800">
                  Maintenance mode is <strong>active</strong>. Public forms are
                  blocked.
                </div>
              )}
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Control how many form submissions a single IP can make. Protects
                against bot spam.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setSettings((p) => ({
                        ...p,
                        rate_limit_forms_window_minutes: Math.max(
                          1,
                          parseInt(e.target.value) || 10,
                        ),
                      }))
              ...
              <p className="text-xs text-muted-foreground">
                Currently: <strong>{settings.rate_limit_forms_max}</strong>{" "}
                submissions per{" "}
                <strong>{settings.rate_limit_forms_window_minutes}</strong>{" "}
                minute
                {settings.rate_limit_forms_window_minutes !== 1 ? "s" : ""} per
                IP. Changes take effect immediately after saving.
              </p>
            </CardContent>
          </Card>
          ...
          >
            <Card
              className={`transition-colors hover:border-border ${menuOpen ? "border-blue-200 bg-blue-50/30" : ""}`}
            >
              <CardContent className="flex items-center justify-between py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Show or hide sidebar navigation items
                    </p>
                  </div>
                </div>
                <ChevronRightIcon
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure media paths and upload limits. Changes are saved
                automatically.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDisplayPrefs({ mediaBasePath: e.target.value })
                  }
                ...
                <p className="mt-1.5 text-xs text-muted-foreground">
                  ...
                  <code className="rounded bg-accent px-1">
                    {displayPrefs.mediaBasePath || "/uploads/portfolio"}
                    /filename.webp
                  </code>
                </p>
              </div>
              <div>
                ...
                />
                <p className="mt-1.5 text-xs text-muted-foreground">
                  Ensure your server and reverse proxy (e.g. nginx{" "}
                  <code className="rounded bg-accent px-1">
                    client_max_body_size
                  </code>
                  ) allow at least this size. Backend default is 10 MB via{" "}
                  <code className="rounded bg-accent px-1">MAX_FILE_SIZE</code>{" "}
                  env var.
                </p>
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Active provider: <strong>{storage.active_provider}</strong>. S3
                credentials are only required when using the S3 provider.
            ...
            {storage.active_provider === "s3" && (
              <CardContent className="space-y-4">
                <SecretField
                  label="Access Key ID"
                  value={storage.s3_access_key_id}
                  onChange={(v) =>
                    setStorage((p) => ({ ...p, s3_access_key_id: v }))
                  }
                />
                <SecretField
                  label="Secret Access Key"
                  value={storage.s3_secret_access_key}
                  onChange={(v) =>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setStorage((p) => ({ ...p, s3_bucket: e.target.value }))
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setStorage((p) => ({ ...p, s3_region: e.target.value }))
                      }
                    />
                  </div>
                </div>
            ...
            {storage.active_provider === "local" && (
              <CardContent>
                <p className="text-sm text-muted-foreground">
                  Using local filesystem storage. Files are saved to{" "}
                  <code className="rounded bg-accent px-1 text-xs">
                    src/public/uploads/
                  </code>
                  . Switch to S3 via the{" "}
                  <code className="rounded bg-accent px-1 text-xs">
                    STORAGE_PROVIDER=s3
                  </code>{" "}
                  env var.
          ...
          <div className="hidden lg:block max-w-2xl flex-1 sticky top-8">
            ...
            <div className="space-y-3">
              {menuStructure.map((group) => {
                const toggleable = group.items.filter((i) => i.key !== null);
                const visibleCount =
                  toggleable.filter((i) => menuPrefs[i.key!] !== false).length +
                  group.items.filter((i) => i.key === null).length;
                const isCollapsed = menuCollapsed[group.group];
                const allVisible =
                  toggleable.length === 0 ||
                  toggleable.every((i) => menuPrefs[i.key!] !== false);
                      ...
                      <CardContent className="pt-0 pb-3 px-4">
                        <ul className="space-y-1">
                          {group.items.map((item) => {
                            ...
                            const visible =
                              locked || menuPrefs[item.key!] !== false;
                            return (
                              <li
                                key={item.name}
                              ...
                              >
                                ...
                                <input
                                  ...
                                  disabled={locked}
                                  onChange={() =>
                                    !locked && toggleMenuItem(item.key!)
                                  }
