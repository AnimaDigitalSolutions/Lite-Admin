  ...
  async (req, res, next) => {
    try {
      ...
      if (!invoice)
        return res
          .status(404)
          .json({ error: { message: "Invoice not found", status: 404 } });
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const invoiceNumber =
        data.invoice_number || (await db.invoices.nextNumber());
      ...
      const createdItems = invoice.id
        ? await db.invoiceItems.findByInvoiceId(invoice.id)
        : [];
      ...
      res
        .status(201)
        .json({ success: true, data: { ...invoice, items: createdItems } });
    } catch (error) {
      next(error);
    }
  },
