...
export default function ComposePage() {
  ...
  const [addToContactEmails, setAddToContactEmails] = useState<Set<string>>(
    new Set(),
  );
  ...
  const nonContactToRecipients = [
    ...
    (r) =>
      !r.isContact &&
      !contacts.some((c) => c.email.toLowerCase() === r.email.toLowerCase()),
  );
          ...
          <CardContent className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                <span
                  className={
                    step === "write" ? "text-foreground font-medium" : ""
                  }
                >
                  Write
                </span>
                ...
                <span
                  className={
                    step === "preview" ? "text-foreground font-medium" : ""
                  }
                >
                  Preview
                </span>
                ...
                <span
                  className={
                    step === "confirm" ? "text-foreground font-medium" : ""
                  }
                >
                  Send
                </span>
            ...
            {step === "preview" && (
              <>
                ...
                {ccRecipients.length > 0 && (
                  <RecipientPreview label="CC" recipients={ccRecipients} />
                )}
                {bccRecipients.length > 0 && (
                  <RecipientPreview label="BCC" recipients={bccRecipients} />
                )}
                <div>
                  <label className="text-xs font-medium text-muted-foreground block mb-1">
                    Subject
                  </label>
                  <div className="px-3 py-2 bg-muted rounded-md text-sm border border-border font-medium">
                    {subject}
                  </div>
                </div>
                <div>
                  ...
                  <div className="px-3 py-3 bg-muted rounded-md text-sm border border-border whitespace-pre-wrap min-h-[100px]">
                    {body}
                  </div>
                </div>
              </>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-medium text-amber-800">
                      You are about to send a real email
                    </p>
                    ...
                    {addToContactEmails.size > 0 && (
                      <p className="text-sm text-amber-700 mt-1">
                        {addToContactEmails.size} new recipient(s) will be added
                        to your Contacts after sending.
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div>
                {step !== "write" && (
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      setStep(step === "confirm" ? "preview" : "write")
                    }
                    disabled={sending}
                  >
                    Back
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1.5 px-3 py-2 bg-muted rounded-md border border-border">
        {recipients.map((r) => (
          <span
            key={r.email}
            className="inline-flex items-center gap-1.5 rounded-full pl-1 pr-2.5 py-0.5 text-xs font-medium bg-card border border-border"
          >
            <span
              className={`inline-flex items-center justify-center h-5 w-5 rounded-full text-[10px] font-bold text-white shrink-0 ${avatarColor(r.name || r.email)}`}
            >
