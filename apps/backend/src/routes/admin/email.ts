...
router.post(
  ...
  async (req, res, _next) => {
    try {
      ...
      const allToEmails = to.map((r: { email: string }) =>
        r.email.toLowerCase(),
      );
      ...
      for (const c of contacts) {
        contactsByEmail.set(
          (c as { email: string }).email.toLowerCase(),
          c as { id: number },
        );
      }
      ...
      return res.json({
        success: true,
        data: { email_sent: true, to: allToEmails },
      });
    } catch (error) {
      const msg =
    ...
    try {
      ...
      if (!name || typeof name !== "string" || !validNames.includes(name)) {
        return res.status(400).json({
          error: {
            message: `Invalid template name. Must be one of: ${validNames.join(", ")}`,
            status: 400,
          },
        });
      }
      ...
      data: {
        email: {
          ...
          ahasend_account_id:
            ...
            "",
          resend_api_key:
            overrides["email_resend_api_key"] ??
        ...
        storage: {
          ...
          s3_access_key_id:
            overrides["storage_s3_access_key_id"] ??
            config.storage.s3?.accessKeyId ??
            "",
          s3_secret_access_key:
            overrides["storage_s3_secret_access_key"] ??
            config.storage.s3?.secretAccessKey ??
            "",
          s3_bucket:
            overrides["storage_s3_bucket"] ?? config.storage.s3?.bucket ?? "",
          s3_region:
            overrides["storage_s3_region"] ?? config.storage.s3?.region ?? "",
        },
      },
    });
  } catch (error) {
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      if (email) {
        if (typeof email.active_provider === "string") {
          await settingsService.set("email_provider", email.active_provider);
          emailChanged = true;
        }
        if (typeof email.ahasend_api_key === "string") {
        ...
        if (typeof email.ahasend_account_id === "string") {
          await settingsService.set(
            "email_ahasend_account_id",
            email.ahasend_account_id,
          );
          emailChanged = true;
        }
        if (typeof email.resend_api_key === "string") {
          await settingsService.set(
        ...
        if (typeof email.from_address === "string") {
          await settingsService.set("email_from", email.from_address);
          emailChanged = true;
        }
        if (typeof email.display_name === "string") {
          await settingsService.set("email_display_name", email.display_name);
          emailChanged = true;
        }
        if (typeof email.notification_address === "string") {
          await settingsService.set(
      ...
      if (storage) {
        if (typeof storage.s3_access_key_id === "string")
          await settingsService.set(
            "storage_s3_access_key_id",
            storage.s3_access_key_id,
          );
        if (typeof storage.s3_secret_access_key === "string")
          await settingsService.set(
            "storage_s3_secret_access_key",
            storage.s3_secret_access_key,
          );
        if (typeof storage.s3_bucket === "string")
          await settingsService.set("storage_s3_bucket", storage.s3_bucket);
        if (typeof storage.s3_region === "string")
          await settingsService.set("storage_s3_region", storage.s3_region);
      }

      await db.adminLogs.create({
        action: "credentials_update",
        resource: "system",
        details: "Provider credentials updated",
        ip_address: req.ip,
      });
...
// Verify API key
router.post(
  "/credentials/verify-key",
  validateBody(verifyKeySchema),
  async (req, res, next) => {
    try {
      ...
      try {
        if (provider === "resend") {
          ...
          return res.status(400).json({
            error: { message: `Unknown provider: ${provider}`, status: 400 },
          });
        }
      } catch (fetchErr) {
