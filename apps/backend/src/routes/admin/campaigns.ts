  ...
  async (req, res, next) => {
    try {
      ...
      const campaign = await db.campaigns.create({
        ...
        target_tags:
          target_tags && target_tags.length
            ? JSON.stringify(target_tags)
            : undefined,
      });
  ...
  async (req, res, next) => {
    try {
      ...
      if (!campaign)
        return res
          .status(404)
          .json({ error: { message: "Campaign not found", status: 404 } });
  ...
  async (req, res, next) => {
    try {
      ...
      if (target_tags !== undefined)
        data.target_tags = JSON.stringify(target_tags);
    ...
    try {
      ...
      if (!deleted)
        return res.status(404).json({
          error: {
            message: "Campaign not found or already sent",
            status: 404,
          },
        });
  ...
  async (req, res, next) => {
    try {
      ...
      if (campaign.status !== "draft")
        return res.status(400).json({
          error: { message: "Campaign has already been sent", status: 400 },
        });
      ...
      if (campaign.target_type === "tagged" && campaign.target_tags) {
        ...
        subscribers = tags.length
          ? await db.waitlist.findActiveByTags(tags)
          : await db.waitlist.findAllActive();
      } else {
        subscribers = await db.waitlist.findAllActive();
      }
      if (subscribers.length === 0) {
        return res.status(400).json({
          error: { message: "No active subscribers to send to", status: 400 },
        });
      }
      ...
      const activeProvider =
        settingsService.get("email_provider") ?? config.email.provider;
      ...
      for (const sub of subscribers) {
        try {
          ...
          logger.error({
            message: `Failed to send campaign to ${sub.email}`,
            error: err,
          });
        }
      }
