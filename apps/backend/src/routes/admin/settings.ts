  ...
  try {
    ...
    res.json({
      data: {
        ...
        rate_limit_forms_window_minutes:
          settingsService.getRateLimitFormsWindowMinutes(),
      },
    });
  } catch (error) {
    next(error);
    ...
    try {
      ...
      if (typeof rate_limit_forms_max === "number") {
        await settingsService.set(
          "rate_limit_forms_max",
          String(rate_limit_forms_max),
        );
      }
      if (typeof rate_limit_forms_window_minutes === "number") {
        await settingsService.set(
          "rate_limit_forms_window_minutes",
          String(rate_limit_forms_window_minutes),
        );
      }
...
router.get("/settings/menu", async (_req, res, next) => {
  try {
    const settingsService = await SettingsService.getInstance();
    const all = settingsService.getAll();
