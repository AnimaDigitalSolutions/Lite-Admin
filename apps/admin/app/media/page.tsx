...
function getTypeBadge(mimeType: string): { label: string; color: string } {
  ...
  switch (type) {
    case "image":
      return { label: "IMG", color: "bg-blue-100 text-blue-800" };
    case "video":
      return { label: "VID", color: "bg-purple-100 text-purple-800" };
    case "document":
      return { label: "DOC", color: "bg-amber-100 text-amber-800" };
    default:
      return { label: "FILE", color: "bg-accent text-foreground" };
  }
}
...
export default function MediaPage() {
  ...
  const handleBulkDownload = () => {
    const items = filteredAndSortedItems.filter((item) =>
  ...
  const handleEdit = async (
    item: MediaItem,
    data: { filename?: string; project_name?: string; description?: string },
  ) => {
    try {
  ...
  const handleSelectAll = (checked: boolean) => {
    checked
      ? selectAll(filteredAndSortedItems.map((i) => i.id))
      : clearSelection();
  };
  ...
  const navigatePreview = (direction: number) => {
    ...
    const idx = filteredAndSortedItems.findIndex(
      (i) => i.id === previewItem.id,
    );
  ...
  const filteredAndSortedItems = sortItems(
    mediaItems.filter((item) => {
      ...
      const matchesType =
        filterType === "all" || getMediaType(item.mime_type) === filterType;
  ...
  const uniqueProjects = Array.from(
    new Set(mediaItems.map((item) => item.project_name).filter(Boolean)),
  );
      ...
      <div className="space-y-6">
          ...
          <div>
            ...
            <p className="mt-1 text-muted-foreground">
              ...
              {(searchTerm || filterProject || filterType !== "all") &&
                ` (filtered from ${mediaItems.length})`}
            </p>
          </div>
          {/* Bulk actions */}
          {selectedIds.size > 0 && (
          ...
          <CardContent>
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-1">
                ...
                Documents: PDF · Max {prefs.maxUploadSizeMB || 10} MB
              </p>
            </div>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {/* Type badge */}
                    <span
                      className={`absolute top-2 left-2 px-1.5 py-0.5 rounded text-[10px] font-bold ${badge.color}`}
                    >
                      {badge.label}
                    </span>
                    {/* Selection checkbox */}
                  ...
                  <tbody>
                    {filteredAndSortedItems.map((item) => {
                      ...
                      return (
                        <tr
                          key={item.id}
                          className={`border-b hover:bg-accent even:bg-muted/50 group/row ${isSelected ? "bg-blue-50" : ""}`}
                        >
                          <td className="p-3">
                            <div
                              className={`${isSelected ? "opacity-100" : "opacity-0 group-hover/row:opacity-100"} transition-opacity`}
                            >
                              <input
                                type="checkbox"
                                checked={isSelected}
                          ...
                          <td className="p-3 text-sm text-muted-foreground text-right tabular-nums">
                            {formatFileSize(item.file_size)}
                          </td>
                          ...
                          <td
                            ...
                            title={formatDateFull(item.uploaded_at)}
                          >
        ...
        {previewItem &&
          (() => {
            ...
            const currentIdx = filteredAndSortedItems.findIndex(
              (i) => i.id === previewItem.id,
            );
            ...
            return (
              ...
              >
                ...
                <div className="absolute top-4 left-4 text-white/80 text-sm z-10">
                  ...
                  <span className="ml-3 text-white/50">
                    {currentIdx + 1} / {filteredAndSortedItems.length}
                  </span>
                </div>
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10">
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      void copyMediaPath(previewItem);
                    }}
                  >
                    <ClipboardDocumentIcon className="h-4 w-4 mr-1" />
                    Copy Path
