  ...
  async (req, res, next) => {
    try {
      ...
      if (!updated)
        return res
          .status(404)
          .json({ error: { message: "Site not found", status: 404 } });

      await db.adminLogs.create({
        action: "site_key_regenerate",
        resource: "sites",
        resource_id: id,
  ...
  async (req, res, next) => {
    try {
      ...
      if (typeof is_active === "boolean")
        await db.sites.toggleActive(id, is_active);
      const site = await db.sites.findById(id);
      res.json({ success: true, data: site });
    } catch (error) {
      next(error);
...
router.put(
  ...
  validateParams(idParamSchema),
  validateBody(sitePermissionsSchema),
  async (req, res, next) => {
    try {
      const db = await DatabaseService.getInstance();
      const id = req.validatedParams.id;
      const { permissions } = req.validatedBody;
      const updated = await db.sites.updatePermissions(
        id,
        permissions.join(","),
      );
      if (!updated)
      ...
      await db.adminLogs.create({
        action: "site_permissions_update",
        resource: "sites",
        resource_id: id,
        details: `Permissions set to: ${permissions.join(", ")}`,
        ip_address: req.ip,
      });
      const site = await db.sites.findById(id);
      res.json({ success: true, data: site });
