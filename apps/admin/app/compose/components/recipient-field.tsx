...
export default function RecipientField({
  ...
  const filteredContacts = useMemo(() => {
    const available = contacts.filter(
      (c) => !allUsedEmails.has(c.email.toLowerCase()),
    );
  ...
  if (isValidEmail && !addedEmails.has(query.trim().toLowerCase())) {
    if (
      !filteredContacts.some(
        (c) => c.email.toLowerCase() === query.trim().toLowerCase(),
      )
    ) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "Enter") {
      ...
      if (options.length > 0 && showDropdown) {
        ...
        if (opt.type === "contact" && opt.contact)
          addRecipient({
            email: opt.contact.email,
            name: opt.contact.name,
            isContact: true,
          });
        else if (opt.type === "raw" && opt.email)
          addRecipient({ email: opt.email });
      ...
      <div className="flex gap-1.5">
        ...
        >
          {recipients.map((r) => (
            ...
            >
              <span
                className={`inline-flex items-center justify-center h-5 w-5 rounded-full text-[10px] font-bold shrink-0 text-white ${avatarColor(r.name || r.email)}`}
              >
          ...
          <input
            ...
            placeholder={
              recipients.length === 0 ? "Type a name or email..." : ""
            }
            ...
            {options.map((opt, i) => (
              ...
              >
                {opt.type === "contact" ? (
                  <>
                    <span
                      className={`inline-flex items-center justify-center h-8 w-8 rounded-full text-white text-xs font-bold shrink-0 ${avatarColor(opt.contact!.name)}`}
                    >
                  ...
                  <>
                    <span className="inline-flex items-center justify-center h-8 w-8 rounded-full bg-muted text-muted-foreground text-xs shrink-0">
                      @
                    </span>
                    <div className="min-w-0 flex-1">
            ...
            {contacts.length > filteredContacts.length && (
              ...
              >
                ...
                <span className="text-sm font-medium">
                  Browse all {contacts.length} contacts...
                </span>
              </button>
            )}
          </div>
