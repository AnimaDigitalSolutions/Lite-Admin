...
export default function EmailPage() {
  ...
  const [verifying, setVerifying] = useState<Record<Provider, boolean>>({
    ahasend: false,
    resend: false,
  });
  const [verified, setVerified] = useState<Record<Provider, VerifyState>>({
    ahasend: null,
    resend: null,
  });
  ...
  const handleVerify = async (provider: Provider) => {
    const key =
      provider === "ahasend" ? config.ahasend_api_key : config.resend_api_key;
    if (!key.trim()) return;
  ...
  const handleTest = async () => {
    ...
    try {
      if (testType === "contact") {
        ...
        await emailTestApi.testWaitlist({
          test_email: testEmail,
          name: "Test User",
        });
      }
      setTestResult({
        ok: true,
        message: `Test ${testType} email sent to ${testEmail}`,
      });
    } catch {
      setTestResult({
        ok: false,
        message:
          "Failed to send test email. Check your API key and from address.",
      });
    } finally {
      setTesting(false);
    }
  ...
  return (
    <ProtectedLayout>
      <div className="max-w-2xl space-y-6">
        <PageHeader
          title="Email"
          description="Configure how outgoing emails are sent and where admin notifications go."
        />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Select your active provider and enter its credentials. Only the
              active provider&apos;s key is used.
            </CardDescription>
          </CardHeader>
            ...
            {providers.map(({ id, label }) => {
              ...
              return (
                <div
                  key={id}
                  onClick={() =>
                    setConfig((prev) => ({ ...prev, active_provider: id }))
                  }
                    ...
                    {id === "ahasend" && (
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            set("ahasend_account_id")(e.target.value)
                          }
                    ...
                    <div className="flex items-end gap-2">
                      <div className="flex-1">
                        <SecretField
                          ...
                          placeholder={
                            id === "resend" ? "re_…" : "Your AhaSend key"
                          }
                        />
                      </div>
                      <Button
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              The <strong>From</strong> address and name that recipients see on
          ...
          <CardContent className="space-y-4">
            ...
            <p className="rounded bg-muted px-3 py-2 text-xs text-muted-foreground border">
              ...
              <span className="font-mono">
                {config.display_name || "Lite Admin"} &lt;
                {config.from_address || "noreply@…"}&gt;
              </span>
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            ...
            {!config.notification_address && (
              <p className="mt-2 text-xs text-amber-600 bg-amber-50 rounded px-3 py-2 border border-amber-200">
                No notification address set, so contact form alerts will fall back
                to the From address.
              </p>
            )}
          </CardContent>
          ...
          >
            <div>
              ...
              <CardDescription className="mt-1">
                Verify your config is working before going live.
              </CardDescription>
            </div>
            <ChevronDownIcon
              className={`h-5 w-5 shrink-0 text-muted-foreground transition-transform duration-200 ${testOpen ? "rotate-180" : ""}`}
            />
          </button>
          {testOpen && (
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    value={testType}
                    onChange={(e) =>
                      setTestType(e.target.value as "contact" | "waitlist")
                    }
