...
function requireSiteKey(scope: string): RequestHandler {
  return async (req, res, next) => {
    ...
    if (!key) {
      res
        .status(401)
        .json({ error: { message: "Missing X-Site-Key header", status: 401 } });
      return;
    }
    ...
    if (!site) {
      res.status(401).json({
        error: { message: "Invalid or inactive site key", status: 401 },
      });
      return;
    }
    const permissions = (site.permissions ?? "contact,waitlist")
      .split(",")
      .map((s) => s.trim());
    if (!permissions.includes(scope)) {
      res.status(403).json({
        error: {
          message: `Site key not authorized for: ${scope}`,
          status: 403,
        },
      });
      return;
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const result = await contactService.processSubmission(req.validatedBody, {
        ip: req.ip || "unknown",
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const result = await waitlistService.addToWaitlist(req.validatedBody, {
        ip: req.ip || "unknown",
        userAgent: req.get("user-agent"),
        siteId: res.locals["siteId"] as number | undefined,
      });
