  ...
  return (
    <Card>
      ...
      >
        ...
        <ChevronDownIcon
          className={`h-5 w-5 shrink-0 text-muted-foreground transition-transform duration-200 ${showTestEmail ? "rotate-180" : ""}`}
        />
      </button>
      {showTestEmail && (
        <CardContent className="space-y-4 border-t border-border pt-4">
          ...
          {testEmailResult && (
            <div
              className={`p-3 rounded-md ${testEmailResult.success ? "bg-green-50 text-green-800 border border-green-200" : "bg-red-50 text-red-800 border border-red-200"}`}
            >
          ...
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              onClick={() => {
                setTestEmailData({ test_email: "", name: "Jane Smith" });
                setTestEmailResult(null);
              }}
            >
              Reset Form
